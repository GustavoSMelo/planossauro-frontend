...
const handleChangeFiles = (fileList: FileList): void => {
    ...
    const validFiles = helper.filter(file =>
        ['image/png', 'image/jpeg', 'application/pdf', 'image/webp'].includes(file.type)
    );
...
const hasEmptyFields = (): boolean => !title.value.length || !category.value.length || !description.value.length;
        ...
        <div v-else class="supportContent">
            ...
            <form>
                ...
                <div v-if="files.length" class="listContainer">
                    <li v-for="(item, index) in files" class="itemList">
                        <span class="itemContainer" v-if="item.type === 'application/pdf'">
                            <small>{{ item.name.slice(0, 8) }}.{{ item.type.split('/')[1] }}</small>
                            <img class="image" src="../assets/pdf.png" alt="pdf icon" />
                            <i class="pi pi-trash" @click="handleRemoveFileFromList(index)"></i>
                        </span>
